...
@section('content')
    ...
    <div id="edithargamodal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-white mt-32 pb-8">
                <button type="button"
                    ...
                    data-modal-toggle="edithargamodal">
                ...
                <div class="py-6 px-6 lg:px-8">
                    <h3 class="mt-5 mb-3 text-2xl text-center font-bold text-gray-900 dark:text-gray-900">Edit Harga Koin
                    </h3>
                    <form method="POST" action="{{ url('finance/edit-harga/1') }}">
                        ...
                        <h3 class="mt-5 mb-3 text-xs text-center text-gray-900 dark:text-gray-900">Harga setiap pembayaran
                            akan berbeda karena mengikuti pajak</h3>
                        <div class="flex flex-row mt-5 justify-center px-8">
                            <button type="submit"
                                class="bg-green-500 hover:bg-green-600 text-white p-3 w-full rounded-lg">Simpan</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <div id="pelatihanmodal" tabindex="-1" aria-hidden="true"
        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full justify-center items-center">
        <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow dark:bg-white mt-32 pb-8">
                <button type="button"
                    class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white"
                    data-modal-toggle="pelatihanmodal">
                    <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                        xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd"
                            d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                ...
                <div class="py-6 px-6 lg:px-8">
                    <h3 class="mt-5 mb-3 text-2xl text-center font-bold text-gray-900 dark:text-gray-900">Edit Harga
                        Pelatihan Skill Level</h3>
        ...
        <table class="table-auto w-full mt-5">
            ...
            <tbody>
                @foreach ($product as $data)
                    <tr>
                        <td class="text-center">{{ $data->id }} <input type="text" name="id[]"
                                value="{{ $data->id }}" hidden></td>
                        <td>
                            <p>{{ $data->name }}</p>
                            <p>{{ $data->description }}</p>
                        </td>
                        <td class="text-center"><input type="number" value="{{ $data->price }}"
                                class="bg-gray-100 rounded-lg border-gray-300" name="price[]"
                                onkeyup="sum({{ $no }})" id="price{{ $no }}">
                        </td>
                        <td class="text-center">
                            <label class="inline-flex relative items-center cursor-pointer align-middle mr-5">
                                <input type="checkbox" value="1" class="sr-only peer" name="tombol[]"
                                    id="tombol{{ $no }}" onclick="sum({{ $no }})"
                            ...
                            <input type="number" value="{{ $data->promo }}"
                                class="bg-gray-100 rounded-lg border-gray-300" name="promo[]"
                                id="promo{{ $no }}" onkeyup="sum({{ $no }})">
                        </td>
                        <td class="text-center">
                            <input type="number" id="total{{ $no }}" value="{{ $data->total }}"
                                name="" disabled>
                            <input type="number" id="totall{{ $no }}" value="{{ $data->total }}"
                                name="total[]" hidden>
                        </td>
                    </tr>
                    @php
