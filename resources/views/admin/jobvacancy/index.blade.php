...
@section('content')
    <section>
        <div class="flex justify-between items-center mb-5">
            <form action="{{ route('admin.lowongan.index') }}" method="get" class="flex space-y-0 space-x-2">
            ...
            @can('create-job-vacancy')
                <div>
                    <a class="btn btn-primary" href="{{ route('admin.lowongan.create') }}">Add New</a>
                </div>
            @endcan
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5">
            @forelse ($jobs as $job)
                <div class="card-group">
                    <div class="body">
                        <div class="title">
                            <a href="{{ route('public.lowongan.showWithSlug', [$job->id, $job->slug]) }}" target="_blank">{{ $job->title }}</a>
                        </div>
                        <div class="desc mt-5">
                            <div class="jobdesc">
                                <div class="font-semibold">Main Skill:</div>
                                <div>
                                    <a href="{{ route('public.lowongan.indexSkill', $job->main_skill->slug) }}" target="_blank">{{ $job->main_skill->name }}</a>
                                </div>
                            </div>
                            <div class="jobdesc">
                            ...
                            <div class="jobdesc">
                                ...
                                <div>
                                    <a href="{{ route('public.lowongan.indexLocation', $job->address['provinsi']) }}" target="_blank" id="{{ 'provinsi' . $job->id }}"></a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="footer">
                        <div class="information">
                            <div class="author">
                                ...
                                <div>
                                    <div class="name"><a href="{{ route('public.mitra.showWithSlug', [$job->mitra->id, Str::slug($job->mitra->user->name)]) }}" target="_blank">{{ $job->mitra->user->name }}</a></div>
                                    <div class="created">{{ date_format(date_create($job->deadline), 'd F Y H:i') }}</div>
                                </div>
                            </div>
                            <div class="another">
                                @can('edit-job-vacancy')
                                    <a href="{{ route('admin.lowongan.edit', $job->id) }}" class="btn btn-small btn-secondary"><i class="fas fa-edit"></i></a>
                                @endcan
                                @can('delete-job-vacancy')
                                    <form action="{{ route('admin.lowongan.destroy', $job->id) }}" method="post" onsubmit="return confirm('Are you sure?');" class="space-y-0">
                                        @csrf
                                        @method('delete')
